...
veryOldInventors.push(
  inventors.filter((man) => {
    return man.year > 1500 && man.year < 1600;
  })
);
...
inventorNames.push(
  inventors.map((man) => {
    return { first: man.first, last: man.last };
  })
);
...
isAdultPresent = devs.some((human) => {
  return human.year < 2006;
});
...
isEveryone19OrOlder = devs.every((human) => {
  return human.year < 2005;
});
...
// Enter your solution code here:
commentById = comments.find((talk) => {
  return talk.id === 823423;
});
...
// Enter your solution code here:
idx = comments.findIndex((talk) => {
  return talk.id === 123523;
});
...
// Enter your solution code here:
totalYearsLived = inventors.reduce((total, guy) => {
  total += guy.passed - guy.year;
  return total;
}, 0);
...
travelMethodCounts = travelMethods.reduce((total, vehicle) => {
  const type = vehicle;
  if (!total[type]) {
    total[type] = 1;
  } else {
